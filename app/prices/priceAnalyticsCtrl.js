(function() {
	...
	function PriceAnalyticsCtrl($scope, $filter, products, prodactService){
		$scope.title = "Price Analytics";
		
		for(var i=0; i<products.length; i++)
		{
			products[i].marginPercent = 
				prodactService.calculateMarginPercent(products[i].price, products[i].cost);
			
			products[i].marginAmount = 
				prodactService.calculateMarginAmount(products[i].price, products[i].cost);	
		}
